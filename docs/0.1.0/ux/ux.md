...
A user visits the Claudette home screen, sees an ASCII-art branded banner and a grid of existing projects. They click **"+ New Project"**, which opens a modal asking for a project name and a host directory path. On submission, the backend initialises the directory with `.ai-factory/` scaffolding, runs `git init`, and registers the project. The user is taken into the project workspace at the **Vision** stage.
...
- **Claudette ASCII banner** with gradient title, subtitle, and version label.
...
2. **StagesSidebar** (left): Stages list with status icons, token counts per stage, and a reset button on hover. Claudette ASCII branding at the bottom.
