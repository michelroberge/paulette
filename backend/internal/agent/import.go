...
func StreamImport(ctx context.Context, hostDir, version, projectName string, emit func(StreamEvent)) (*ImportResult, error) {
	...
	// Step 6: Review (programmatic — baseline import)
	emit(StreamEvent{Type: "log", Content: "Generating review artifact..."})
	artifacts[model.StageReview] = buildImportReviewArtifact(projectName)
	emit(StreamEvent{Type: "artifact", Content: "review"})
...
func buildImportReviewArtifact(projectName string) string {
	return fmt.Sprintf(`# Review Report: %s

## Coverage Assessment

This is a baseline import review. The artifacts were auto-generated by analyzing the existing codebase.

### Vision Coverage
The vision document was reverse-engineered from the code. It should be reviewed for accuracy and completeness.

### UX Coverage
User journeys were inferred from routes, pages, and UI components. Manual review is recommended.

### Architecture Coverage
The architecture document reflects the current codebase structure. It should be validated against actual deployment and infrastructure.

## Issues Found
- Auto-generated artifacts may not capture all business context or design rationale
- Some implicit behaviors may not be documented

## Recommendations
- Review each artifact carefully and refine via chat before approving
- Pay special attention to the Vision document — it captures *what* exists but may miss *why*
- Use the Enhancement flow after approval to iterate on the project

## Final Verdict
Ready for user review`, projectName)
}
