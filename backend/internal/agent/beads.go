...
const parseBuildPlanSystemPrompt = `You are a Build Plan Parser for an AI App Factory. Read the build plan below and extract all milestones and tasks into a structured JSON format.
...
func ParseBuildPlan(ctx context.Context, buildMdContent string) (<-chan StreamEvent, error) {
	var prompt strings.Builder
	prompt.WriteString("Build Plan:\n---\n")
	prompt.WriteString(buildMdContent)
	prompt.WriteString("\n---\n\nExtract all milestones and tasks from this build plan into the required JSON format.")
...
func ExecuteBead(ctx context.Context, projectDir string, bead model.Bead, artifacts map[model.StageName]string) (<-chan StreamEvent, error) {
